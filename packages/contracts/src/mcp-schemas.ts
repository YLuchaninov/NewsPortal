...
const candidateSignalGroupSchema = {
  type: "object",
  required: ["name", "cues"],
  properties: {
    name: stringSchema,
    tier: { type: "string", enum: ["context", "buyer_intent", "project_intent"] },
    cues: { type: "array", items: stringSchema },
  },
  additionalProperties: false,
} satisfies JsonSchema;
const candidateSignalGroupListSchema = {
  type: "array",
  items: candidateSignalGroupSchema,
} satisfies JsonSchema;
...
export const MCP_TEMPLATE_PAYLOAD_SCHEMAS = {
  systemInterestCreate: {
    ...
    properties: {
      ...
      candidate_positive_signal_groups: candidateSignalGroupListSchema,
      candidate_negative_signal_groups: candidateSignalGroupListSchema,
      ...
      selection_profile_auto_select_mode: {
        type: "string",
        enum: ["disabled", "evidence_led", "llm_approved", "evidence_or_llm"],
      },
      selection_profile_signal_visibility: {
        type: "string",
        enum: ["explicit_marker", "hidden_intent", "mixed", "unknown"],
      },
      selection_profile_auto_select_min_positive_groups: flexibleIntegerSchema,
      selection_profile_auto_select_min_cue_hits: flexibleIntegerSchema,
      selection_profile_auto_select_requires_no_noise: booleanSchema,
      selection_profile_auto_select_requires_no_technical_veto: booleanSchema,
  ...
  llmTemplateCreate: {
    ...
    properties: {
      ...
      purpose: {
        type: "string",
        enum: ["selection_review", "structured_extraction", "classification", "scoring"],
      },
