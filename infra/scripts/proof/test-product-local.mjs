...
async function runProductCommand(item, options) {
  ...
  return evaluateLocalProductCommandResult({
    key: item.key,
    lane: item.lane,
    command: [item.executable, ...item.args].join(" "),
    proves: item.proves ?? [],
    weakAllowed: Boolean(item.weakAllowed),
...
function buildIncludedLanes(mode) {
  ...
  return [
    ...coreLanes,
    "discovery",
    "live-website-matrix-diagnostic",
    "mcp-live-http",
  ];
}
...
function formatMarkdown(report) {
  ...
  if (report.commands.length > 0) {
    lines.push("", "## Commands", "");
    lines.push("| Key | Lane | Raw Status | Acceptance | Exit | Command |");
    lines.push("| --- | --- | --- | --- | --- | --- |");
    for (const item of report.commands) {
      lines.push(
        `| ${item.key} | ${item.lane} | \`${item.status}\` | \`${item.acceptanceStatus ?? item.status}\` | ${item.exitCode} | \`${item.command}\` |`
      );
    }
  }
  ...
  if (report.strictLiveInternet) {
    lines.push("", "## Strict Live", "");
    lines.push("- `SIGNALOPS_STRICT_LIVE_INTERNET=1` was active, so weak live diagnostics are hard failures.");
  }
...
async function main() {
  ...
  const strictLiveInternet = isStrictLiveInternet({ ...env, ...process.env });
  const commands = [];
  ...
  if (envResult.status === "failed") {
    ...
    for (const item of commandList) {
      ...
      const result = await runProductCommand(item, { strictLiveInternet });
      commands.push(result);
      if (args.failFast && result.acceptanceStatus === PRODUCT_LOCAL_STATUSES.failed) {
  ...
  const diagnosticWeak = commands
    .filter((item) => item.acceptanceStatus === PRODUCT_LOCAL_STATUSES.weak)
    .map((item) => ({
      key: item.key,
      reason: item.acceptanceReason ?? item.diagnosticSummary?.reason ?? "classified_weak_diagnostic",
      verdictCounts: item.diagnosticSummary?.verdictCounts ?? null,
    }));
  const skippedCommands =
    envResult.status === "failed" || args.preflightOnly
      ...
      : [];
  const status = determineLocalProductStatus({
  ...
  const report = await writeArtifacts({
    ...
    env: envResult,
    strictLiveInternet,
    commands: stripParsedArtifacts(commands),
    skippedCommands,
    diagnosticWeak,
