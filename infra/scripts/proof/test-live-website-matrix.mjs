...
function parseCliArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    if (arg === "--help") {
      console.log(
        [
          "Usage: node infra/scripts/proof/test-live-website-matrix.mjs [--variant=<variantKey>] [--group=<groupKey>] [--site=<candidateName>]",
...
async function main() {
  ...
  const firebaseApiKey = requireConfigured(env, "FIREBASE_WEB_API_KEY", {
    proofName: "live website validation",
  });
