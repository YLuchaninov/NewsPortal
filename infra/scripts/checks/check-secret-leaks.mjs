...
const patterns = [
  ...
  },
  {
    ...
    regex: /\bBearer\s+[A-Za-z0-9._~+/=-]{32,}/,
  },
  {
    name: "firebase_service_account_private_key_json",
    regex: new RegExp(`"private_key"\\s*:\\s*"-{5}${"BEGIN"} PRIVATE KEY-{5}`),
  },
];
...
for (const file of files) {
  if (file === ".env.prod") {
    issues.push(".env.prod must not be tracked; keep real beta secrets outside git.");
    continue;
  }
