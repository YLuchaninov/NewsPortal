...
const forbiddenCopyPatterns = [
  ...
  /^\.ruff_cache(?:\/|$)/,
  /^build(?:\/|$)/,
  /^runtime\/node\/apps\/[^/]+\/dist(?:\/|$)/,
  /^runtime\/node\/apps\/[^/]+\/\.astro(?:\/|$)/,
  /^runtime\/node\/services\/[^/]+\/dist(?:\/|$)/,
  /^data(?:\/|$)/,
  /^docs(?:\/|$)/,
  /^infra\/scripts(?:\/|$)/,
  /^node_modules(?:\/|$)/,
...
const packageRoots = ["runtime/node/packages", "runtime/node/apps", "runtime/node/services"];
const issues = [];
const compiledRuntimePackages = new Map([
  [
    "runtime/node/services/fetchers/package.json",
    [
  ...
  ["runtime/node/services/mcp/package.json", ["start"]],
  ["runtime/node/services/relay/package.json", ["migrate", "start"]],
]);
...
const nodeRuntimeDockerfiles = new Map([
  ["infra/docker/admin.Dockerfile", /CMD\s+\["node",\s*"build\/node\/apps\/admin\/server\/entry\.mjs"\]/],
  ["infra/docker/fetchers.Dockerfile", /CMD\s+\["node",\s*"build\/node\/services\/fetchers\/main\.mjs"\]/],
  ["infra/docker/mcp.Dockerfile", /CMD\s+\["node",\s*"build\/node\/services\/mcp\/main\.mjs"\]/],
  ["infra/docker/relay.Dockerfile", /CMD\s+\["node",\s*"build\/node\/services\/relay\/main\.mjs"\]/],
  ["infra/docker/web.Dockerfile", /CMD\s+\["node",\s*"build\/node\/apps\/web\/server\/entry\.mjs"\]/],
]);
...
const finalNodeRuntimeForbiddenPatterns = [
  /\bCOPY\b(?![^\n]*--from=)[^\n]*(?:^|\s)runtime\/node\/(?:services|packages|apps)\/[^/\s]+\/src(?:\s|\/|$)/im,
  /\bCOPY\b(?![^\n]*--from=)[^\n]*(?:^|\s)build(?:\s|\/|$)/im,
...
function checkCompiledRuntimeEntrypoints() {
  for (const [file, scriptNames] of compiledRuntimePackages) {
    ...
    for (const scriptName of scriptNames) {
      ...
      if (script.includes("--import tsx") || /\bsrc\/.+\.ts\b/.test(script)) {
        issues.push(`${file} script "${scriptName}" must run compiled build output, not TS source.`);
      }
      if (!/\bbuild\/node\/.+\.mjs\b/.test(script)) {
  ...
  for (const file of compiledRuntimeDockerfiles) {
    ...
    if (!/CMD\s+\["node",\s*"build\/node\/services\/.+\.mjs"\]/.test(text)) {
      issues.push(`${file} CMD must run a compiled build .mjs entrypoint directly.`);
    }
  }
  ...
  for (const [file, expectedCommandPattern] of nodeRuntimeDockerfiles) {
    ...
    if (!expectedCommandPattern.test(text)) {
      issues.push(`${file} CMD must run its compiled build entrypoint directly.`);
    }
