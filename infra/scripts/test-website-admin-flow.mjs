...
async function main() {
  ...
  try {
    ...
    const apiChannelName = `API admin acceptance ${runId}`;
    log("Creating an API channel through the admin surface.");
    const adminApiChannel = await postForm(
      "http://127.0.0.1:4322/bff/admin/channels",
      {
        providerType: "api",
        name: apiChannelName,
        fetchUrl: `http://127.0.0.1:${fixtureServer.port}/api/items`,
        language: "en",
        maxItemsPerPoll: "10",
        requestTimeoutMs: "5000",
    ...
    const apiChannelId = String(adminApiChannel.json?.channelId ?? "");
    if (!apiChannelId) {
      throw new Error("Admin API channel creation did not return a channelId.");
    }

    await assertHtmlContains("http://127.0.0.1:4322/channels", [apiChannelName, "API"], {
      cookie: adminCookie,
    });
    await assertHtmlContains(
      `http://127.0.0.1:4322/channels/${encodeURIComponent(apiChannelId)}/edit`,
      ["Edit API channel", "JSON field mapping", "Authorization header"],
      { cookie: adminCookie }
    );

    log("Triggering a deterministic API fetch with admin-configured auth.");
    runCompose(
      "exec",
      "-T",
      "fetchers",
    ...
    const apiFetchedChannel = await waitFor(
      "API channel fetch success",
      async () =>
        fetchJson(`http://127.0.0.1:8000/channels/${encodeURIComponent(apiChannelId)}`),
      (payload) =>
        String(payload?.channel_id ?? "") === apiChannelId &&
        Number(payload?.stored_item_count ?? 0) >= 1 &&
        String(payload?.last_success_at ?? "").trim().length > 0
    );

    await waitFor(
      "API authorization header capture",
      async () => fixtureServer.readState(),
      (payload) =>
        Array.isArray(payload?.apiAuthorizationHeaders) &&
        payload.apiAuthorizationHeaders.includes(apiAuthorizationHeader)
    );

    const apiUpdatedName = `${apiChannelName} updated`;
    log("Updating the API channel through the admin surface.");
    await postForm(
      "http://127.0.0.1:4322/bff/admin/channels",
      {
    ...
    const updatedApiChannel = await waitFor(
      "API channel update reflection",
      async () =>
        fetchJson(`http://127.0.0.1:8000/channels/${encodeURIComponent(apiChannelId)}`),
      (payload) =>
        String(payload?.name ?? "") === apiUpdatedName &&
        payload?.is_active === false &&
        payload?.has_authorization_header === true
    );

    const emailPassword = "MailboxSecret!";
    const emailChannelName = `Email IMAP admin acceptance ${runId}`;
    log("Creating an Email IMAP channel through the admin surface.");
    const adminEmailImapChannel = await postForm(
      "http://127.0.0.1:4322/bff/admin/channels",
      {
        ...
        port: "993",
        secure: "true",
        username: `alerts-${runId}@example.test`,
        password: emailPassword,
        mailbox: "INBOX",
        searchFrom: "press@example.test",
        maxItemsPerPoll: "8",
        enrichmentEnabled: "true",
        enrichmentMinBodyLength: "500",
      },
    ...
    const emailImapChannelId = String(adminEmailImapChannel.json?.channelId ?? "");
    if (!emailImapChannelId) {
      throw new Error("Admin Email IMAP channel creation did not return a channelId.");
    }

    await assertHtmlContains(
      "http://127.0.0.1:4322/channels",
      [emailChannelName, "Email IMAP"],
      { cookie: adminCookie }
    );
    await assertHtmlContains(
      `http://127.0.0.1:4322/channels/${encodeURIComponent(emailImapChannelId)}/edit`,
      ["Edit Email IMAP channel", "Current password status", "Mailbox"],
      { cookie: adminCookie }
    );

    const emailImapUpdatedName = `${emailChannelName} updated`;
    log("Updating the Email IMAP channel while preserving the stored password.");
    await postForm(
      "http://127.0.0.1:4322/bff/admin/channels",
      {
        channelId: emailImapChannelId,
        providerType: "email_imap",
        name: emailImapUpdatedName,
        language: "en",
        isActive: "false",
        pollIntervalSeconds: "900",
        adaptiveEnabled: "false",
        maxPollIntervalSeconds: "5400",
        host: "imap.example.test",
        port: "993",
        secure: "true",
        username: `alerts-${runId}@example.test`,
        mailbox: "PRESS",
        searchFrom: "alerts@example.test",
        maxItemsPerPoll: "6",
        enrichmentEnabled: "false",
        enrichmentMinBodyLength: "700",
      },
      {
        cookie: adminCookie,
      }
    ...
    const updatedEmailImapChannel = await waitFor(
      "Email IMAP channel update reflection",
      async () =>
        fetchJson(`http://127.0.0.1:8000/channels/${encodeURIComponent(emailImapChannelId)}`),
      (payload) =>
        String(payload?.name ?? "") === emailImapUpdatedName &&
        payload?.is_active === false &&
        String(payload?.fetch_url ?? "") === "imaps://imap.example.test:993/PRESS" &&
        String(payload?.config_json?.mailbox ?? "") === "PRESS" &&
        String(payload?.config_json?.password ?? "") === emailPassword
    );
      ...
      JSON.stringify(
        {
          ...
          apiChannelId,
          apiChannelStoredItemCount: apiFetchedChannel.stored_item_count,
          apiChannelUpdatedName: updatedApiChannel.name,
          emailImapChannelId,
          emailImapFetchUrl: updatedEmailImapChannel.fetch_url,
          providerChannelsVerified: ["website", "api", "email_imap"],
        },
        null,
        2
      )
