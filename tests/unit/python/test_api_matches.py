...
class ApiMatchesTests(unittest.TestCase):
    def test_list_user_matches_uses_ranked_dedup_query(self) -> None:
        ...
        with (
            patch.object(api_main, "query_one", return_value={"total": 3}) as query_one,
        ...
        self.assertIn("matched.family_rank = 1", count_sql)
        ...
        self.assertIn(
            "order by matched_items.published_at desc nulls last, matched_items.ingested_at desc nulls last, matched_items.content_item_id",
            items_sql,
        )
    ...
    def test_list_user_matches_supports_search_and_title_sort(self) -> None:
        with (
        ...
        count_sql, count_params = query_one.call_args.args
        self.assertIn(
            "where coalesce(matched_items._search_text, '') ilike %s escape '\\'",
            count_sql,
        )
        self.assertEqual(count_params, ("user-1", "%AI policy%"))
        ...
        self.assertIn(
            "order by matched_items._normalized_title desc nulls last",
            items_sql,
        )
