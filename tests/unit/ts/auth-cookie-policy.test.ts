...
function withCookieSecurePolicy<T>(
  ...
  const previous = process.env.SIGNALOPS_COOKIE_SECURE_POLICY;
  if (value == null) {
    delete process.env.SIGNALOPS_COOKIE_SECURE_POLICY;
  } else {
    process.env.SIGNALOPS_COOKIE_SECURE_POLICY = value;
  }
  ...
  try {
    ...
    if (previous == null) {
      ...
      process.env.SIGNALOPS_COOKIE_SECURE_POLICY = previous;
    }
  }
}

test("cookie secure policy normalizes invalid env values to auto", () => {
  assert.equal(readCookieSecurePolicy({ SIGNALOPS_COOKIE_SECURE_POLICY: "always" }), "always");
  assert.equal(readCookieSecurePolicy({ SIGNALOPS_COOKIE_SECURE_POLICY: "never" }), "never");
  assert.equal(readCookieSecurePolicy({ SIGNALOPS_COOKIE_SECURE_POLICY: "AUTO" }), "auto");
  assert.equal(readCookieSecurePolicy({ SIGNALOPS_COOKIE_SECURE_POLICY: "surprise" }), "auto");
});

test("cookie secure policy follows request scheme and forwarded proto in auto mode", () => {
  assert.equal(
    shouldMarkCookieSecure({
      env: { SIGNALOPS_COOKIE_SECURE_POLICY: "auto" },
