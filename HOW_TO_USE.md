...
Каналы — это источники новостей. В текущем MVP админка уже умеет работать с двумя operator-ready типами:
...
Другие provider types (`api`, `email_imap`, `youtube`) уже присутствуют в архитектуре, но в текущей админке еще не имеют полноценного operator create/edit flow.

### 4.1. Добавление одного канала

В текущем MVP на странице каналов есть отдельные create/edit flows как для **RSS**, так и для **Website**.
...
В левой колонке для RSS используется форма **Add RSS Channel**:
...
Если вам нужно добавить много RSS-каналов сразу — используйте **Bulk Import** (средняя колонка).
...
>
> `Bulk Import` выше относится именно к RSS JSON. Website-источники обычно настраиваются по одному, потому что для них часто нужно указать homepage/seed URLs, discovery mode flags и при необходимости browser fallback.
...
> Да. В текущем MVP уже поддерживаются `website` channels через отдельный operator flow в админке. Для них главным экраном проверки становится `Resources`, а для JS-heavy сайтов можно opt-in включить browser-assisted fallback. `api`, `email_imap` и `youtube` по-прежнему остаются follow-up provider types без полноценного operator create/edit flow в текущем admin baseline.
