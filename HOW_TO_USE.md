...
```
RSS-ленты → Сбор статей → Очистка текста → Удаление дубликатов → Группировка событий → Системные критерии → AI-проверка серой зоны для критериев → System Feed → Дополнительная персонализация → Уведомления
```
...
5. Только после этого, если у конкретного пользователя настроены личные интересы, система выполняет дополнительную персонализацию и может отправить уведомление (через браузер, Telegram или email)
...
2. **Шаблоны интересов** — библиотека системных тем, которые синхронизируются в live `criteria` для всего продукта
...
Шаблоны интересов — это **библиотека готовых тем**, которую администратор настраивает для всей системы. Активные шаблоны синхронизируются в live `criteria` и становятся частью **системного слоя отбора** статей.
...
**Аналогия:** Шаблоны интересов — это редакционная политика или каталог тем для всей системы. А `user_interests` — это уже персональная подписка конкретного человека внутри этого потока.
