...
- Kind: Stage
- Status: completed
- In scope: add a shared admin section card class and replace exact repeated `rounded-2xl border border-border bg-card p-5 shadow-sm` strings across selected admin pages, discovery tabs and editor form components.
- Out of scope: hero cards, table containers, cards with different padding/background/border semantics, copy, layout semantics, visual redesign, page data loading, routes and runtime services.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/lib/admin-ui-classes.ts`, `apps/admin/src/pages/channels.astro`, `apps/admin/src/pages/resources.astro`, `apps/admin/src/pages/user-interests.astro`, `apps/admin/src/pages/analysis.astro`, `apps/admin/src/pages/analysis-policies.astro`, `apps/admin/src/pages/filter-policies.astro`, `apps/admin/src/components/discovery/*`, `apps/admin/src/components/channel-editor-form-parts.tsx`, `apps/admin/src/components/LlmTemplateEditorForm.tsx`, `apps/admin/src/components/InterestTemplateEditorForm.tsx`.
- Risk: low-medium, because this touches many UI surfaces, but it only replaces exact class strings with a shared constant while preserving DOM structure and component props.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that replaced class strings are exact matches.
- Acceptance criteria: selected surfaces no longer repeat the exact section-card class string inline.
- Architecture note: affected concern is admin UI primitive cohesion; stakeholder/consumer is admin operators and maintainers; tradeoff is a shared class constant instead of cross-framework component churn.
- Implemented, with evidence: `ADMIN_SECTION_CARD_CLASS` was added to `apps/admin/src/lib/admin-ui-classes.ts`.
- Implemented, with evidence: selected admin pages, discovery tabs and editor form components now use the shared section-card class for exact `rounded-2xl border border-border bg-card p-5 shadow-sm` matches.
- Scope note: hero cards, table containers, cards with different padding/background/border semantics, copy, layout semantics, visual redesign, page data loading, routes and runtime services were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: replaced class strings were exact matches; the one advanced hints details kept its `group` class while composing the shared section card class.

### AIDP-ENGINEERING-REFACTORING-UNIFICATION-STAGE-64

- Kind: Stage
- Status: completed
- In scope: reuse `ADMIN_COMPACT_METRIC_TILE_CLASS` as the shared base for flex metric tiles in channels, resources and discovery control-plane hero surfaces while keeping each tile's local `flex min-h-* flex-col` behavior.
- Out of scope: non-flex metric tiles, table classes, buttons, form controls, copy, layout semantics, visual redesign, page data loading, routes and runtime services.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/pages/channels.astro`, `apps/admin/src/pages/resources.astro`, `apps/admin/src/components/discovery/DiscoveryControlPlaneHero.astro`.
- Risk: low, because this only moves the exact repeated base class string into a shared constant and leaves each tile's unique flex/min-height classes in place.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that base class strings are exact matches and local flex/min-height classes remain.
- Acceptance criteria: touched flex metric tiles no longer repeat the compact metric tile base class string inline.
- Architecture note: affected concern is admin UI primitive cohesion; stakeholder/consumer is admin operators and maintainers; tradeoff is composing a shared class string in Astro rather than introducing cross-page components.
- Implemented, with evidence: channels, resources and discovery control-plane hero flex metric tiles now compose `ADMIN_COMPACT_METRIC_TILE_CLASS`.
- Scope note: non-flex metric tiles, table classes, buttons, form controls, copy, layout semantics, visual redesign, page data loading, routes and runtime services were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: base class strings were exact matches and local `flex min-h-* flex-col` classes remain on every changed tile.
...
- Kind: Stage
- Status: completed
- In scope: replace identical local `resolveListHref(nextPage)` wrappers in admin interest and LLM template list pages with the shared `createPageHrefResolver`.
- Out of scope: view-tab href helpers, template actions, pagination component behavior, query parameter policy, page data loading, API calls, auth/session behavior, routes, layout, copy, visual redesign and runtime services.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/pages/templates/interests.astro`, `apps/admin/src/pages/templates/llm.astro`.
- Risk: low, because this only binds the existing shared page resolver with the same `Astro.url` and `DEFAULT_PAGE`.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that template list page query preservation and default-page deletion remain equivalent.
- Acceptance criteria: touched template pages no longer define duplicated local `resolveListHref(nextPage)` functions.
- Architecture note: affected concern is admin template page helper cohesion; stakeholder/consumer is admin operators and maintainers; tradeoff is reusing the existing page resolver binder instead of repeated local wrapper functions.
- Implemented, with evidence: interest and LLM template list pages now use `createPageHrefResolver(Astro.url, { defaultPage: DEFAULT_PAGE })`.
- Scope note: view-tab href helpers, template actions, pagination component behavior, query parameter policy, page data loading, API calls, auth/session behavior, routes, layout, copy, visual redesign and runtime services were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: template list page query preservation and default-page deletion remain equivalent because the new resolver delegates to the same shared page-href logic with the same URL/default page.
...
- Kind: Stage
- Status: completed
- In scope: move AutomationEditorWorkspace sequence metadata panel into a focused `automation-editor-sequence-settings.tsx` component.
- Out of scope: save/build handler bodies, workflow state, graph transforms, selected-task inspector rendering, advanced JSON fallback, header/palette/canvas/dialog rendering, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/AutomationEditorWorkspace.tsx` and new `apps/admin/src/components/automation-editor-sequence-settings.tsx`.
- Risk: medium, because metadata fields feed saved workflow definitions, but the stage only moves markup and preserves controlled input wiring.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that metadata copy, field values, placeholders and status options remain equivalent.
- Acceptance criteria: sequence metadata renders through the new component with equivalent controlled fields; save/build logic remains in the workspace.
- Architecture note: affected concern is admin automation editor cohesion; stakeholder/consumer is admin/operator workflow editing; tradeoff is separating sequence settings presentation from editor orchestration.
- Implemented, with evidence: added `apps/admin/src/components/automation-editor-sequence-settings.tsx` with sequence title, description, status, trigger event, cron, max runs and tags controls.
- Implemented, with evidence: `AutomationEditorWorkspace.tsx` now renders `AutomationEditorSequenceSettings` while save/build handler bodies and state ownership remain local.
- Scope note: save/build handler bodies, workflow state, graph transforms, selected-task inspector rendering, advanced JSON fallback, header/palette/canvas/dialog rendering, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: metadata copy, field values, placeholders and status options remain equivalent through props.

### AIDP-ENGINEERING-REFACTORING-UNIFICATION-STAGE-44

- Kind: Stage
- Status: completed
- In scope: move AutomationEditorWorkspace advanced JSON fallback tab content into a focused `automation-editor-advanced-json.tsx` component.
- Out of scope: graph parsing/apply handler body, workflow state, graph transforms, save/run/archive handlers, header/palette/canvas/inspector/dialog rendering, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/AutomationEditorWorkspace.tsx` and new `apps/admin/src/components/automation-editor-advanced-json.tsx`.
- Risk: medium, because the fallback can rewrite the canvas graph, but the stage only moves markup and preserves the apply callback.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that advanced tab copy, textarea binding and apply action remain equivalent.
- Acceptance criteria: advanced JSON fallback renders through the new component with equivalent input and action; `applyAdvancedJson` remains in the workspace.
- Architecture note: affected concern is admin automation editor cohesion; stakeholder/consumer is admin/operator workflow editing; tradeoff is separating fallback presentation from graph orchestration.
- Implemented, with evidence: added `apps/admin/src/components/automation-editor-advanced-json.tsx` with the advanced JSON fallback copy, task graph textarea and Apply JSON To Canvas action.
- Implemented, with evidence: `AutomationEditorWorkspace.tsx` now renders `AutomationEditorAdvancedJson` while `applyAdvancedJson` and graph parsing/apply logic remain local.
- Scope note: graph parsing/apply handler body, workflow state, graph transforms, save/run/archive handlers, header/palette/canvas/inspector/dialog rendering, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: advanced tab copy, textarea binding and apply action remain equivalent through props.

### AIDP-ENGINEERING-REFACTORING-UNIFICATION-STAGE-43

- Kind: Stage
- Status: completed
- In scope: move AutomationEditorWorkspace manual run dialog into a focused `automation-editor-run-dialog.tsx` component.
- Out of scope: run request handler body, JSON parsing/submission behavior, workflow state, graph transforms, header/palette/canvas/inspector rendering, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/AutomationEditorWorkspace.tsx` and new `apps/admin/src/components/automation-editor-run-dialog.tsx`.
- Risk: medium, because the dialog triggers a workflow run, but the stage only moves markup and preserves submit/cancel wiring.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that dialog labels, textarea bindings, open state and submit/cancel handlers remain equivalent.
- Acceptance criteria: run dialog renders through the new component with equivalent inputs and actions; `handleRunNow` and JSON parsing remain in the workspace.
- Architecture note: affected concern is admin automation editor cohesion; stakeholder/consumer is admin/operator workflow execution; tradeoff is separating manual-run presentation from run orchestration.
- Implemented, with evidence: added `apps/admin/src/components/automation-editor-run-dialog.tsx` with the manual run dialog, context JSON textarea, trigger meta JSON textarea and cancel/request actions.
- Implemented, with evidence: `AutomationEditorWorkspace.tsx` now renders `AutomationEditorRunDialog` while `handleRunNow` and JSON parsing/submission remain local.
- Scope note: run request handler body, JSON parsing/submission behavior, workflow state, graph transforms, header/palette/canvas/inspector rendering, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: dialog labels, textarea bindings, open state and submit/cancel handlers remain equivalent through props.
...
- Kind: Stage
- Status: completed
- In scope: move AutomationEditorWorkspace node library palette/sidebar into a focused `automation-editor-palette.tsx` component.
- Out of scope: workflow state, graph transforms, save/run/archive handlers, canvas/inspector/dialog rendering, plugin grouping/filtering algorithms, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/AutomationEditorWorkspace.tsx` and new `apps/admin/src/components/automation-editor-palette.tsx`.
- Risk: medium, because the palette creates workflow steps, but the stage only moves markup and preserves the append callback.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that palette labels, classes, template href, search binding and append callback remain equivalent.
- Acceptance criteria: node library renders through the new component with equivalent search and append behavior; plugin grouping/filtering stays owned by the workspace.
- Architecture note: affected concern is admin automation editor cohesion; stakeholder/consumer is admin/operator workflow editing; tradeoff is separating palette presentation from graph orchestration.
- Implemented, with evidence: added `apps/admin/src/components/automation-editor-palette.tsx` with the node library card, search field, template link and plugin append buttons.
- Implemented, with evidence: `AutomationEditorWorkspace.tsx` now renders `AutomationEditorPalette` and keeps plugin grouping/filtering plus append logic local.
- Scope note: workflow state, graph transforms, save/run/archive handlers, canvas/inspector/dialog rendering, plugin grouping/filtering algorithms, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: palette labels, classes, template href, search binding and append callback remain equivalent through props.
...
- Kind: Stage
- Status: completed
- In scope: move AutomationEditorWorkspace React Flow node renderers and nodeTypes mapping into a focused `automation-editor-flow-nodes.tsx` module.
- Out of scope: workflow state, graph transforms, save/run/archive handlers, inspector form fields, palette rendering, React Flow props other than nodeTypes import, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/AutomationEditorWorkspace.tsx` and new `apps/admin/src/components/automation-editor-flow-nodes.tsx`.
- Risk: medium, because the node renderers are user-facing workflow UI, but the stage only moves presentational markup and preserves React Flow nodeTypes behavior.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that node markup/classes/handles/toolbar copy and nodeTypes keys remain equivalent.
- Acceptance criteria: React Flow nodes render through the new module with the same `input` and `default` type mapping; editor state/save behavior stays untouched.
- Architecture note: affected concern is admin automation editor cohesion; stakeholder/consumer is admin/operator workflow editing; tradeoff is separating canvas node presentation from editor state orchestration.
- Implemented, with evidence: added `apps/admin/src/components/automation-editor-flow-nodes.tsx` containing `WorkflowTriggerNode`, `WorkflowTaskNode` and exported `automationEditorNodeTypes`.
- Implemented, with evidence: `AutomationEditorWorkspace.tsx` now imports `automationEditorNodeTypes` and no longer owns React Flow node renderer markup.
- Scope note: workflow state, graph transforms, save/run/archive handlers, inspector form fields, palette rendering, React Flow props other than nodeTypes import, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: node markup/classes/handles/toolbar copy and `input`/`default` nodeTypes keys remain equivalent in the new module.
...
- Kind: Stage
- Status: completed
- In scope: move RSS advanced fetch/enrichment/adapter controls from `ChannelEditorForm.tsx` into a focused `channel-editor-rss-fields.tsx` component.
- Out of scope: website/API/IMAP provider controls, common polling controls, form field names, default values, option ordering, selected/resolved adapter semantics, validation semantics, hidden fields, submit/cancel behavior, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/ChannelEditorForm.tsx` and new `apps/admin/src/components/channel-editor-rss-fields.tsx`.
- Risk: medium, because RSS adapter and freshness fields affect persisted source-channel behavior, but the stage only moves existing markup and preserves field names/defaults/options.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that RSS field names, defaults, options, placeholders and resolved status copy remain equivalent.
- Acceptance criteria: RSS advanced controls render equivalently from the new component; `ChannelEditorForm.tsx` retains form composition; no payload or validation behavior changes.
- Architecture note: affected concern is admin channel editor provider-family cohesion; stakeholder/consumer is admin/operator RSS source-channel configuration; tradeoff is extracting a cohesive provider branch while preserving compatibility.
- Implemented, with evidence: added `apps/admin/src/components/channel-editor-rss-fields.tsx` with RSS maximum adaptive interval, max items, request timeout, content:encoded preference, adapter strategy, max entry age, enrichment, enrichment min body length, user-agent and resolved status controls.
- Implemented, with evidence: `ChannelEditorForm.tsx` now renders `ChannelEditorRssFields` in the RSS advanced branch and passes the existing selected/resolved adapter and max-entry-age values.
- Scope note: website/API/IMAP provider controls, common polling controls, form field names, default values, option ordering, selected/resolved adapter semantics, validation semantics, hidden fields, submit/cancel behavior, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: RSS field names, defaults, options, placeholders and resolved status copy remain equivalent in the new component.
...
- Kind: Stage
- Status: completed
- In scope: move API JSON field mapping heading and field controls from `ChannelEditorForm.tsx` into `channel-editor-api-mapping-fields.tsx`.
- Out of scope: API request budget/enrichment fields, website/RSS/IMAP provider controls, form field names, default values, validation semantics, hidden fields, submit/cancel behavior, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/ChannelEditorForm.tsx` and new `apps/admin/src/components/channel-editor-api-mapping-fields.tsx`.
- Risk: medium, because JSON mapping fields define persisted API channel payload mapping, but the stage only moves existing markup and preserves field names/defaults.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that API mapping field names, defaults and help text remain equivalent.
- Acceptance criteria: API JSON mapping controls render equivalently from the new component; API request-budget controls stay in the main form for later staged splits; no payload or validation behavior changes.
- Architecture note: affected concern is admin channel editor provider-family cohesion; stakeholder/consumer is admin/operator API source-channel configuration; tradeoff is extracting mapping controls separately from API request/enrichment controls.
- Implemented, with evidence: added `apps/admin/src/components/channel-editor-api-mapping-fields.tsx` with the API JSON mapping heading and `itemsPath`, title, lead, body, URL, published-at, external ID and language field controls.
- Implemented, with evidence: `ChannelEditorForm.tsx` now renders `ChannelEditorApiMappingFields` in the API advanced branch while keeping API request-budget/enrichment controls local.
- Scope note: API request budget/enrichment fields, website/RSS/IMAP provider controls, form field names, default values, validation semantics, hidden fields, submit/cancel behavior, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: API mapping field names, defaults and help text remain equivalent in the new component.
...
- Kind: Stage
- Status: completed
- In scope: add local presentational parts for channel editor overview cards, section shell and sticky actions; rewire `ChannelEditorForm.tsx` to use them.
- Out of scope: form field names, default values, option lists, validation semantics, hidden fields, provider-specific control groups, submit/cancel destinations, copy changes, layout changes, visual redesign, server writes and API/runtime code.
- Allowed paths: `.aidp/work.md`, `apps/admin/src/components/ChannelEditorForm.tsx` and new `apps/admin/src/components/channel-editor-form-parts.tsx`.
- Risk: medium, because the sticky submit/cancel shell is user-facing, but the change only moves existing markup and keeps link href, submit label and default submit behavior equivalent.
- Required proof: `pnpm typecheck`; `pnpm lint`; `git diff --check --`; targeted review that overview/section/action classes and labels remain equivalent.
- Acceptance criteria: overview cards, major section shells and sticky actions render equivalently; provider-specific controls stay in the main form for later staged splits; no field/payload behavior changes.
- Architecture note: affected concern is admin channel editor cohesion; stakeholder/consumer is admin/operator source-channel configuration; tradeoff is extracting local shells before provider-family component decomposition.
- Implemented, with evidence: added `apps/admin/src/components/channel-editor-form-parts.tsx` with `ChannelEditorOverview`, `ChannelEditorSection` and `ChannelEditorActions`.
- Implemented, with evidence: `ChannelEditorForm.tsx` now delegates overview cards, basics/auth section shells and sticky form actions to local parts while leaving provider-specific controls in the main form.
- Scope note: form field names, default values, option lists, validation semantics, hidden fields, provider-specific control groups, submit/cancel destinations, copy, layout, visual redesign, server writes and API/runtime code were not changed.
- Passed proof: `pnpm typecheck` passed with 0 errors and existing Astro hints only.
- Passed proof: `pnpm lint` passed, including TS ESLint and Python ruff.
- Passed proof: `git diff --check --` passed.
- Targeted review: overview/section/action classes and labels remain equivalent, including `Back to channels`, submit label rendering and explicit submit button type.
