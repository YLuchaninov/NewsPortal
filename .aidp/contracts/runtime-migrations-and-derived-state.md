...
- Shared Python API/worker images must not bake local `data` contents into the image; derived `data` is created or mounted at runtime, and the image runs as non-root `signalops`.
