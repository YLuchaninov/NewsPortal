...
`operator.funnel.audit` is the read-only calibration surface for prior manual/example bundles, JSON assets, or admin-tuned configurations. It treats reference evidence as calibration input, not runtime truth, and compares it with live system interests, LLM templates, compile state, residuals, selected/web-visible counts, discovery coverage, and adapter-required gaps. It returns a portable funnel spec, drift findings, and recommended MCP actions only; it must not mutate configuration, start discovery, onboard channels, or queue replay.
`discovery.source_priors.evaluate/apply/list` expose rare-signal source priors. `evaluate` is read-only; `apply` may write only prior annotations into Source Evidence Contract/channel config JSON and must preserve the invariant that prior-only evidence has zero coverage/downstream contribution and cannot select web content.
`operator.report.verify` is the read-only report verification surface. Before final human-facing reports for channel onboarding, discovery runs, source-prior application, cleanup, selection, system health, channel health, website pipeline, selection tuning, content analysis, LLM budget, sequence run, or discovery yield decisions, clients should call it with the relevant entity ids and base claims on its DB-backed counts/statuses rather than mutation responses alone.
`operator.report.verify reportKind=funnel_calibration` verifies the DB-backed state after funnel calibration changes: active interests/templates, compile status, selection residuals, selected/web-visible counts, stale pass-through, missing interest-filter rows, drift buckets, discovery source-role gaps, and adapter-required gaps.
`discovery.runs.start` and source expansion tools have start semantics, not row-only create semantics: the API must enqueue the default Discovery V3 Sequence Runner path and expose dispatch metadata or a dispatch failure, while clients still verify completion through `operator.report.verify` or run read-back. Bounded live provider execution is an explicit per-run operator approval (`providerExecutionEnabled`) and must remain separate from source priors or article-selection policy. `discovery.runs.dispatch_queued` is the bounded repair/continuation path for retained queued discovery rows that were created before dispatch metadata existed; it dispatches rows into `q.sequence` without deleting, archiving, or treating old row state as cleanup.
