...
- для `website` canonical config включает bounded discovery/extraction controls вроде `maxResourcesPerPoll`, `requestTimeoutMs`, `totalPollTimeoutMs`, `crawlDelayMs`, discovery mode flags, `collectionSeedUrls`, allow/block URL patterns, `browserFallbackEnabled` и `maxBrowserFetchesPerPoll`;
...
`web_resources` является canonical storage для website-ingestion truth. Editorial-compatible rows могут дополнительно project-иться в `articles` и запускать existing article pipeline, но resource truth не должен зависеть от того, состоялась ли эта projection. FastAPI maintenance list/detail reads и admin `/resources` list/detail surfaces являются canonical operator observability lane для projected и resource-only website rows.
