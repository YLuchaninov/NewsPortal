...
Для admin dashboard и public feed нужно держать один truthful read contract для feed-eligible новостей.

Current MVP-определение public feed:

- read endpoint должен быть отделен от admin moderation list;
- feed включает только `articles.visibility_state = visible`;
- feed включает только статьи, дошедшие как минимум до `processing_state in ('matched', 'notified')`;
- feed всегда paginated server-side и возвращает truthful `total`.
- user-facing copy для этого read model должна описывать набор как `feed` / `feed-eligible`, а не как `published` или `matched-only`, потому что runtime truth включает обе terminal feed states `matched` и `notified`.
...
- пока rollout compatibility для legacy consumers нужна, list endpoints могут временно сохранять raw-array response при отсутствии `page/pageSize`, но canonical contract для всех новых и migrated consumers остается paginated envelope.
...
6. делает batch existence lookup по persisted `ArticleExternalRef` и canonical URL для текущей пачки, чтобы early-skip exact duplicates до insert path;
7. записывает raw article в PostgreSQL;
8. записывает `ArticleExternalRef`;
9. пишет `outbox_event`.
...
3. проверить `user_profiles.notification_preferences` вместе с enabled `user_notification_channels`;
4. убедиться, что статья не заблокирована на момент финального decision/send;
5. определить `new event` или `major update`;
6. создать запись в `notification_log`;
7. передать задачу в dispatch.
...
- manual reindex/backfill jobs обязаны явно различать rebuild-only и DB-repair modes; DB-repair mode должен заново считать `criterion_match_results` / `interest_match_results`, переигрывать gray-zone LLM review с текущими templates и не отправлять retro-notifications молча;
- backfill embeddings;
- replay outbox;
