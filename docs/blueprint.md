...
`web_resources` является canonical storage для website-ingestion truth. Editorial-compatible rows могут дополнительно project-иться в `articles` и запускать existing article pipeline, но resource truth не должен зависеть от того, состоялась ли эта projection. `classification_json` keeps the top-level `kind/confidence/reasons` read contract backward-compatible while also preserving additive discovery/enrichment/resolution/transition truth, and `attributes_json` now carries both generic classifier observability and bounded editorial extraction/body-uplift telemetry. FastAPI maintenance list/detail reads и admin `/resources` list/detail surfaces являются canonical operator observability lane для projected и resource-only website rows.
