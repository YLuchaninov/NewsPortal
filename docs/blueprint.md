...
- feed включает только статьи, для которых `coalesce(system_feed_results.eligible_for_feed, false) = true`;
- `system_feed_results` становится truth-source для public/system feed только после system `criteria` и criteria-scope gray-zone LLM review; optional per-user personalization не может расширять этот upstream gate;
- feed всегда paginated server-side и возвращает truthful `total`.
- user-facing copy для этого read model должна описывать набор как `feed` / `system-selected feed`, а не как `published` или `matched-only`, потому что runtime truth теперь живет в article-level system gate, а не в terminal `processing_state`.
