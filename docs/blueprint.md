...
- Astro BFF write routes в `web` и `admin` обязаны различать browser navigation и scripted/API clients: HTML form flow использует fixed `303` Post-Redirect-Get redirect на root dashboard section с flash status/message и cleanup stale session cookie при auth failure, а non-navigation clients сохраняют machine-readable JSON semantics;
