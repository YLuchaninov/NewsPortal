...
- любой current or future ingestable provider может владеть только acquisition/adaptation слоем до общего handoff; после этого provider-specific pipeline ownership должен заканчиваться на `article.ingest.requested`, а clustering/filtering/selection/read-model truth не должна fork-аться в parallel provider-owned ветку.
- accepted `website` resources после fetchers extraction должны handoff-иться в общий `articles` pipeline или получать explicit pre-pipeline rejection; `web_resources` больше не должны silently оставаться parallel product truth для selected-content surfaces.
- legacy website compat cleanup теперь должен идти через bounded replay of persisted `web_resources`, а не через скрытый recrawl: если работа касается historical website replay, используй explicit projection replay tooling и не маскируй fresh network fetch как будто это deterministic backfill of already persisted rows.
