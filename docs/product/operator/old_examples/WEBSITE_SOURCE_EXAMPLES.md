...
> **Archive status:** historical example-only reference. Active website-source testing guidance now lives in [WEBSITE_SOURCES_TESTING.md](../examples/WEBSITE_SOURCES_TESTING.md). Do not treat the dated live URLs below as current proof without rechecking them.
...
>
> **Что этот документ не покрывает полностью:** полный local MVP runbook, Discovery vNext source acquisition, login-required websites, CAPTCHA bypass, cookie/session replay, stealth scraping и любые гарантии, что внешний сайт не поменяет структуру после 15 апреля 2026.
>
...
- [WEBSITE_SOURCES_TESTING.md](../examples/WEBSITE_SOURCES_TESTING.md)
