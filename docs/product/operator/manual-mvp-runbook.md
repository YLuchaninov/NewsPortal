...
4. Run `pnpm test:discovery:examples:compose` if you want the shipped Example A/B/C profile-backed proof lane and the emitted `manualReplaySettings` that can then be replayed through the discovery overview and focused routes under `/admin/discovery`.
