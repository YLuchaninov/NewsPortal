# EXAMPLES.md — Готовые RSS и template-конфигурации SignalOps для типовых проектов

> **Для кого этот документ:** для администратора, который уже поднял SignalOps и хочет быстро наполнить систему готовыми RSS-каналами, LLM-шаблонами и шаблонами интересов под типовой сценарий.
>
...
## 7. Discovery vNext для этих примеров

Discovery vNext расширяет bundles A/B/C через typed artifacts and source inventory. Он не заменяет ручной импорт стартовых RSS, LLM templates, system interests или downstream filtering; найденный источник сначала проходит `DiscoveryBrief`, candidate acquisition, `ProbeReport`, `SourceUnderstanding` и deterministic `RoutingDecision`, а затем попадает в inventory, adapter backlog или probation handoff.

Для локального proof используйте общий bounded gate:

```sh
pnpm test:discovery-enabled:compose
```

Operator flow теперь один: `/admin/discovery` показывает runs, artifacts, candidates, probe reports, source understanding, routing decisions, source inventory, policies, adapter backlog, replay and rollback. API/MCP writes должны использовать vNext resources under `/maintenance/discovery/*` and `discovery.*`; missing or invalid vNext policy is an error.

Для Examples A/B/C связывайте discovery с уже созданными interests:

1. Сначала импортируйте соответствующий bundle из этого файла.
2. Скомпилируйте `DiscoveryBrief` из active system interests.
3. Запускайте bounded candidate acquisition and probe через vNext policy.
4. Принимайте только routing decisions, которые не используют historical yield as keep/drop signal.
5. Создавайте probation channels только через existing source registrar/outbox handoff, чтобы downstream filtering and notifications остались неизменными.

Вне этого файла остаются полный `.env.dev` bootstrap, admin action-token details, website/browser manual verification and destructive rollback drills.

---

## 8. FAQ по примерам

**В: Можно ли использовать оба набора одновременно в одной установке SignalOps?**
...
> 📖 **Связанные документы:**
> - [HOW_TO_USE.md](../HOW_TO_USE.md) — полное руководство по администрированию SignalOps
