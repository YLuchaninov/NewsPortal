...
- Context model:
  ...
  - reference/manual bundles are calibration evidence, not hidden runtime config; when an operator says an example bundle worked better, compare its source roles, signal families, positive/negative cue groups, content-kind policy and LLM-review guardrails with current MCP state before editing discovery targets, system interests or templates
