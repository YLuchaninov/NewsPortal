...
- Context model:
  ...
  - selection tuning starts by classifying signal visibility: `explicit_marker`, `hidden_intent`, `mixed` or `unknown`; read `signalops://guide/reference/hidden-signal-evidence-lanes`, `signalops://guide/reference/selection-evidence-semantics`, `signalops://guide/playbooks/strict-next-steps`, and `signalops://guide/playbooks/operator-flow-modes` before recommending mutations
  - `must_have_terms` uses any-of runtime matching but remains a hard pre-semantic gate; for hidden or unknown signals the baseline is `must_have_terms=[]` and `short_tokens_required=[]` unless mandatory-marker proof exists
  - `candidateSignals` must use literal observable text fragments in cue groups; concept labels belong in group names, not in cue values; label-like cues, single-cue groups and zero-hit cue groups need representative rejected samples and bounded replay before further tuning
  ...
  - reindex maintenance is MCP-native through `maintenance.reindex.request` plus `maintenance.reindex_jobs.list`; clients should not call `sequences.run` on `Default Reindex` unless they already have a valid reindex job/event context; proof should inspect selection replay counters (`selectionReplayTargetCount`, `selectionReplayedCount`, `skippedSelectionDueToEnrichmentState`) rather than trusting `completed` alone
