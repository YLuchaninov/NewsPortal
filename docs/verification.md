...
| MCP HTTP read matrix | `pnpm test:mcp:http:reads` | Те же предпосылки, что и у `pnpm test:mcp:compose` | Focused deterministic rerun for MCP protocol discovery, shipped resources/prompts, article/content diagnostics, residual-analysis surfaces, read-only operator flows, and doc-parity coverage. |
