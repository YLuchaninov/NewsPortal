...
- для capability closeout вокруг real admin-managed `user_interests` нужен end-to-end compose proof: operator-created interest must compile, fresh ingest must create the targeted `interest_match`, delivery must resolve either by real send or by truthful suppression such as `recent_send_history`, and historical backfill must add the missing match without creating retro notification/suppression drift.
