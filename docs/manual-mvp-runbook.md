...
1. Follow the discovery section in [README.md](/Users/user/Documents/workspace/my/NewsPortal/README.md).
2. Enable `DISCOVERY_ENABLED=1`.
3. Run `pnpm test:discovery-enabled:compose` before trusting a live manual run.
4. If discovery approves a public JS-heavy website candidate with browser assistance recommended, keep the resulting source as `website` and verify any browser provenance on `/resources` instead of treating it as RSS.
