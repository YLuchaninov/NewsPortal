...
const SERVER_INSTRUCTION_LINES = [
  ...
  "For interactive MCP discovery missions, keep payload.maxHypotheses <= 5 unless the operator explicitly accepts a longer async run with payload.confirmLargeRun=true. Treat discovery.missions.run as a run request: verify sequence run/task/candidate state before reporting outcomes.",
