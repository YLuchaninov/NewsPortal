...
const SERVER_INSTRUCTION_LINES = [
  ...
  "For thematic rare-signal acquisition, RSS/web volume is not enough proof of coverage. Use discovery.source_roles.plan/coverage, discovery.adapter_research.plan/list/explain, discovery.indirect_targets.plan, and discovery.indirect_targets.channels.plan for marketplace, ATS, community, forum/support, closed-network, and executable indirect-search lanes.",
  ...
  "Indirect/search channels are acquisition-only API adapters. Materialize them with discovery.indirect_targets.channels.plan, then use channels.bulk_onboard.plan/apply/verify; prove outcomes with operator.report.verify reportKind=indirect_search_execution. For local research, ddgs_search uses the internal API DDGS bridge and remains research_only.",
  ...
  "Experimental adapters are acquisition inputs only. research_only, tosRisk, sourceRole, and adapterKey metadata must stay independent from article selection, ranking, escalation, web visibility, and signal counts.",
  "Discovery v3 uses targets, coverage, runs, hypotheses, endpoints, contracts, claims, negative evidence, provider health, and eval replay. Do not use old mission/recall/candidate discovery tools or route names.",
  "Discovery endpoint promotion requires probe evidence, provider capability, duplicate checks, and a Source Evidence Contract. New direct sources enter trustStage=probation with partial coverage until contract evaluation passes; hidden/social signals create claims and follow-up hypotheses, not direct source_channel promotion.",
  ...
  "After each write, read the affected entity back through MCP. Before final reports, call operator.report.verify for channel_onboarding, discovery_run, cleanup, selection, system_health, channel_health, source_role_coverage, adapter_research, indirect_search_execution, marketplace_extraction_quality, website_pipeline, selection_tuning, content_analysis, llm_budget, sequence_run, or discovery_yield claims.",
