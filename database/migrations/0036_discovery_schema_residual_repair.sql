...
select
  ...
  'Recovered placeholder class created by 0036_discovery_schema_residual_repair.',
  'active',
  'graph_seed_only',
  '{rss,website}'::text[],
  'Recovered placeholder class created during residual schema repair.',
  '{}'::jsonb,
  1,
  999,
  jsonb_build_object('repair_migration', '0036_discovery_schema_residual_repair')
from (
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'discovery_missions_latest_portfolio_snapshot_fk'
  ) then
