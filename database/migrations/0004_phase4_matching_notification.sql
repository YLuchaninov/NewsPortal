...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'signal_candidates_primary_media_asset_id_fkey'
  ) then
    alter table signal_candidates
      add constraint signal_candidates_primary_media_asset_id_fkey
      foreign key (primary_media_asset_id)
      references signal_candidate_media_assets (asset_id)
      on delete set null;
  end if;
end $$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'signal_candidates_event_cluster_id_fkey'
  ) then
    alter table signal_candidates
      add constraint signal_candidates_event_cluster_id_fkey
      foreign key (event_cluster_id)
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'event_clusters_centroid_embedding_id_fkey'
  ) then
    alter table event_clusters
      add constraint event_clusters_centroid_embedding_id_fkey
      foreign key (centroid_embedding_id)
      references embedding_registry (embedding_id)
      on delete set null;
  end if;
end $$;
