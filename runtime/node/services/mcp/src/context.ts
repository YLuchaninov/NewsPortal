...
const SERVER_INSTRUCTION_LINES = [
  ...
  "Before recommending mutations, writes, or final claims, default/autonomous clients must call operator.flow.route or use a prompt/tool output that includes the same route block. Report flowMode, intent fields, mustRead, mustDoNext, doNotDoYet, blockedUntil and proofRequired before proposing changes.",
  "operator.flow.route is read-only advisory routing. It does not prove a mutation occurred, does not grant write authority, and does not make operationMode required on write tools.",
  "Before recommending mutations, choose and report a flowMode from operator.flow.route or signalops://guide/playbooks/operator-flow-modes: diagnostic, planned_change, expert_override, source_onboarding, scenario_pack_rollout, or cleanup.",
  "For system updates, config updates, tuning and cleanup, also choose and report advisory intent fields from signalops://guide/playbooks/change-intents: changeIntent, cleanupIntent, tuningLayer and updateRisk. Intent fields are advisory only and existing write tools do not require them.",
  "changeIntent describes what is changing; tuningLayer describes which pipeline layer is being tuned; cleanupIntent describes what is being cleaned. Mutation responses are not verified effect, and source acquisition proof is not selection proof.",
  "For 0 selected, 0 LLM reviews, Discovery quality gaps, source onboarding, or config writes, first read signalops://guide/playbooks/strict-next-steps and signalops://guide/playbooks/operator-flow-modes. In diagnostic states follow read-back -> classify -> bounded write -> bounded replay -> verify. The strict recommendation fields must_do_next, allowed_after, do_not_do_yet, and blocked_until are prescriptive for default/autonomous clients.",
  "Strict sequencing is a default safety rail for MCP clients, not a ban on expert operator action. Planned improvements use planned_change flow; expert overrides require operatorOverrideReason, affected scope, read-back target, verification target, and cannot skip final read-back/report verification.",
  "System-interest writes must use canonical MCP fields: candidate_positive_signals/candidate_negative_signals or structured candidate_positive_signal_groups/candidate_negative_signal_groups, selection_profile_llm_review_mode, selection_profile_auto_select_mode, selection_profile_signal_visibility, and allowed_content_kinds. Do not send candidateSignals, selectionProfile, llmReviewMode, or allowedContentKinds; those are rejected and are not proof of applied config.",
  ...
  "discovery.brief.preview is diagnostic only. It is not a bypass for domain_contamination or persisted DiscoveryBrief validation; persisted Discovery artifacts must pass validation or be corrected through operator/admin configuration evidence.",
  "When selected count is zero / 0 selected or residuals are dominated by semantic_rejected/no_system_match, do not begin with strictness=broad, mass editing many interests, LLM template rewrites, or adding more RSS volume. Run signalops://guide/scenarios/selection-calibration: residual summary/list, 1-3 representative explains, one interest, candidateSignals-first recovery, bounded docIds replay, and operator.report.verify.",
  "Selection evidence semantics are explicit: must_have_terms is an any-of hard lexical text constraint; short_tokens_required requires extracted short-token features; positive_texts are semantic prototypes, not keyword recovery; criteriaMatches/interestMatches backfill counters are not selected-signal proof.",
  "must_have_terms any-of is still a hard pre-semantic gate. For hidden_intent, mixed hidden lanes, and unknown signals, the baseline is must_have_terms=[] and short_tokens_required=[] unless mandatory marker proof exists.",
  "Classify signalVisibility as explicit_marker, hidden_intent, mixed, or unknown before recommending hard gates. Mixed signals must be split into lane-like system interests, criteria, or scenario-pack entries; hidden lanes must not inherit explicit-marker gates.",
  "candidateSignals group.name is a conceptual label and group.cues must be literal observable text fragments. Snake_case/id-like cues are warnings, not evidence diversity. Candidate signals can auto-select only when selection_profile_auto_select_mode permits it and the generic evidence contract/thresholds are met.",
  "Filter reason counts are result-row counters, not unique candidates. Read operator.selection.dashboard filterReasonBreakdown and compare filterRows with distinctCandidateCount before estimating impact or proposing tuning.",
  "For hard-filter collapse, diagnose technical_filter_rejected before semantic tuning: inspect operator.selection.dashboard filterReasonBreakdown/filterReasonCounts, affected system_interests.read/compile status, 10-30 representative rejected explains, bounded config repair, bounded docIds replay, and operator.report.verify. Multi-word short_tokens_required entries are invalid; use token-like values only.",
  "Do not globally remove generic wrapper/source-navigation filters or switch strictness=broad from row counters alone. Treat broad mode only as a temporary bounded experiment with explicit docIds replay and selected/rejected sample verification.",
  "If active interests exclude the observed signal_candidates.content_kind distribution, fix allowed_content_kinds through admin/MCP config or prove a different projection path. Do not silently relabel RSS/API editorial observations as listings.",
  "For hidden or operational signals, do not recommend positive-term expansion as the primary repair. Use candidateSignals, content/filter policy evidence, near-miss negatives, representative explains, bounded docIds replay, and read-back proof.",
  "Gray-zone collapse is not automatically precision improvement. Check maintenance.reindex_jobs.list, residual distributions, rejected samples, selected samples, hold quality, and operator.report.verify before claiming improved selection.",
  "LLM review mode affects only candidates that reach reviewable or gray-zone paths. llmReviewMode=always does not bypass semantic rejection/no_system_match, and zero LLM spend can mean no_pending_gray_zone rather than a broken provider. Distinguish no reviewable path, review disabled, budget exhausted, worker not running, provider credentials missing, and provider_endpoint_error/provider_404.",
  "Live Discovery without explicit runtime credentials or provider readiness is preflight/not_applicable or runtime_credentials_missing. Do not tune budget to fix missing credentials.",
  ...
  "User-provided session reports and anchored summaries are historical evidence, not current source-health truth. Read channels.bottlenecks.summary/list before reporting active source counts or active failures now.",
  ...
  "For broken or provider-shape-mismatched channels, use channels.bottlenecks.summary/list and channels.alternatives.plan first. RSS alternatives come from the fetchers feed-probe contract; malformed/auth-blocked/not-acceptable/HTML RSS may return same-origin website_fallback candidates with status=needs_probe, which require bulk_onboard plan/apply/verify and must not be auto-created blindly. API-like sources need mapping or a future adapter instead of fake RSS/website onboarding.",
