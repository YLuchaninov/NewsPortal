...
SUPPORTED_PROVIDER_TYPES = set(BETA_INGEST_PROVIDER_TYPE_SET)
...
class PostgresSourceRegistrarAdapter:
    ...
    def register_sources(
        ...
        *,
        ...
        if provider_type not in SUPPORTED_PROVIDER_TYPES:
            raise ValueError(
                f"source registrar expected provider_type to be one of {sorted(SUPPORTED_PROVIDER_TYPES)}."
            )
        if not sources:
            return []
    ...
    def _normalize_source_candidate(
        ...
        *,
        ...
        if effective_provider_type not in SUPPORTED_PROVIDER_TYPES:
            return None
        raw_url = (
            source.get("feed_url")
